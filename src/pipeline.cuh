...
public:
    ...
    bool writeAllOVITO(const vec3D* pos, const vec3D* vel, const vec3D* force, const vec3D* torque, const vec3D* omega, const vec3D* mag, const int* cluserIDs, const double* amon, const int* matID, int Nmon, int Nsto)
    {
        ...
        for (int i = 0; i < Nsto; i++)
        {
            int index = i % Nmon;
            double a = amon[index];
            double distance = cpu_vec3D_length(pos[i]) + a;
        ...
        for (int i = 0; i < Nmon; i++)
        {
            if (vel != 0)
            {
                double len_vel = cpu_vec3D_length(vel[i]);
                if (max_vel < len_vel)
            ...
            if (force != 0)
            {
                double len_force = cpu_vec3D_length(force[i]);
            ...
            if (torque != 0)
            {
                double len_torque = cpu_vec3D_length(torque[i]);
            ...
            if (omega != 0)
            {
                double len_omega = cpu_vec3D_length(omega[i]);
        ...
        for (int i = 0; i < steps; i++)
        {
            ...
            for (int j = 0; j < Nmon; j++)
            {
                ...
                if (vel != 0)
                {
                    double len_vel = cpu_vec3D_length(vel[i * Nmon + j]);
                    if (len_vel > 0)
                    {
                ...
                if (force != 0)
                {
                    double len_force = cpu_vec3D_length(force[i * Nmon + j]);
                    if (len_force > 0)
                    {
                ...
                if (torque != 0)
                {
                    double len_torque = cpu_vec3D_length(torque[i * Nmon + j]);
                    if (len_torque > 0)
                    {
                ...
                if (omega != 0)
                {
                    double len_omega = cpu_vec3D_length(omega[i * Nmon + j]);
                    if (len_omega > 0)
                    {
